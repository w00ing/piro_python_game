...
def board_print(board):  # 보드판 네모로 출력해주는 함수
    ...
    print(board[6], "\n")
    print(
        board[13], "          아파트:땅 가격의 10%, 주택:땅 가격의 20%, 통행료:총 가격의 20%        ", end='')
    print(board[7], "\n")
...
def go(player, dice_1, dice_2):
    ...
    board_rerange(player, old_location,
                  player_location[player])  # 보드판을 바꿔주는 함수
...
def Golden_key(player):  # <황금열쇠  목록>  (이건 랜덤으로 진행)
    ...
    if g == 1:  # 1. 자가격리 대상자입니다.  (-)
        ...
        # 도시 클래스 이용.
        # 4. 원하는 도시를 선택하세요! (+)
        print("자유여행 이용권에 당첨됐습니다. 원하는 도시의 숫자를 입력하세요. (황금열쇠 제외) (단, 월급은 지급되지 않습니다.)")
        ...
        for player in range(1, 4):
            ...
            if(location == 7):
                ...
                if city.owner == player:
                    print(
                        f"현재 위치에서 아파트({city.apt_price} MSK) 또는 주택({city.house_price} MSK)를 지을 수 있습니다.")
                    i = 0
                    while(i == 0):
                        print("건물을 짓겠습니까? (y/n)")
                        answer = input()
